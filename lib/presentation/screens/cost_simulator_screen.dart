...
class _CostSimulatorScreenState extends State<CostSimulatorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    const primaryBlue = Color(
        0xFF197FE6); // Blue for LCL logic if we wanted distinct, but sticking to main theme is better.
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                        _isFCL
                            ? "Calcula impuestos para carga marítima FCL."
                            : "Cálculo optimizado para carga suelta LCL.",
                        style:
                        ...
                        children: [
                          Expanded(
                              child: _buildToggleItem(
                                  "FCL (Contenedor)", true, activeColor)),
                          Expanded(
                              child: _buildToggleItem(
                                  "LCL (Suelta)", false, activeColor)),
                        ],
                      ),
                    ),
                    ...
                    if (_isFCL) ...[
                      FadeInDown(
                        child: _buildCard(
                            surfaceDark, surfaceHighlight, activeColor, [
                          _buildCardTitle(Icons.local_shipping,
                              "Configuración de Carga", activeColor),
                          const SizedBox(height: 16),
                          const Text("Tipo de Contenedor",
                              style: TextStyle(
                                  color: Colors.white,
                                  fontSize: 14,
                          ...
                          const SizedBox(height: 16),
                          const Text("Cantidad de Contenedores",
                              style: TextStyle(
                                  color: Colors.white,
                                  fontSize: 14,
                                  fontWeight: FontWeight.w500)),
                    ...
                    _buildCard(surfaceDark, surfaceHighlight, activeColor, [
                      _buildCardTitle(Icons.payments, "Valores de Importación",
                          activeColor),
                      const SizedBox(height: 16),
                      _buildLabel("Incoterm"),
                      _buildDropdown(["FOB", "CIF", "EXW", "DDP"],
                          surfaceHighlight, activeColor),
                      const SizedBox(height: 4),
                      Text("Término de comercio internacional acordado.",
                          style: TextStyle(
                      ...
                      const SizedBox(height: 16),
                      _buildCheckbox(
                          "Incluir Impuesto ISD (5%)",
                          _includeISD,
                          (v) => setState(() => _includeISD = v!),
                          activeColor,
                          surfaceHighlight),
                    ]),
                    ...
                    _buildCard(surfaceDark, surfaceHighlight, activeColor, [
                      _buildCardTitle(Icons.category,
                          "Clasificación Arancelaria", activeColor),
                      const SizedBox(height: 16),
                      _buildLabel("Producto"),
                      _buildTextInput("Ej: Zapatillas deportivas",
                          _productController, surfaceHighlight, activeColor),
                      const SizedBox(height: 16),
                      Row(
                        children: [
                          Expanded(
                          ...
                          Expanded(
                              ...
                              child: Column(
                                  ...
                                  children: [
                                    ...
                                    _buildTextInput("0", _adValoremController,
                                        surfaceHighlight, activeColor,
                                        suffix: "%", textAlign: TextAlign.right)
                                  ])),
                        ],
                      )
                    ]),
        ...
        child: SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            style: ElevatedButton.styleFrom(
                ...
                foregroundColor: _isFCL ? Colors.black : Colors.white,
                shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(16))),
            icon: const Icon(Icons.calculate),
            label: const Text("Calcular Costos",
  ...
  Widget _buildCard(
      Color bg, Color border, Color accent, List<Widget> children) {
    return Container(
  ...
  Widget _buildChoiceChip(
      String label, bool selected, Color color, VoidCallback onSelect) {
    return GestureDetector(
      onTap: onSelect,
      child: Container(
  ...
  Widget _buildCurrencyInput(
      String label, TextEditingController ctrl, Color bg, Color activeColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildLabel(label),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                contentPadding: const EdgeInsets.only(
                    left: 32, right: 12, top: 12, bottom: 12),
  ...
  Widget _buildTextInput(
      String hint, TextEditingController ctrl, Color bg, Color activeColor,
      {String? suffix, TextAlign textAlign = TextAlign.start}) {
    return Stack(
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            contentPadding: EdgeInsets.only(
                left: 12, right: suffix != null ? 32 : 12, top: 12, bottom: 12),
            border: OutlineInputBorder(
                borderRadius: BorderRadius.circular(12),
                borderSide: const BorderSide(color: Colors.transparent)),
            focusedBorder: OutlineInputBorder(
